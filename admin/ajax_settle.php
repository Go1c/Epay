...
break;
case 'paypwd_check':
	if(isset($_SESSION['paypwd']) && $_SESSION['paypwd']===adminPayPasswordSessionValue($conf['admin_paypwd']))
		exit('{"code":0,"msg":"ok"}');
	else
...
case 'paypwd_input':
	...
	if(verifyConfigSecret($paypwd, $conf['admin_paypwd'], 'admin_paypwd')){
		$_SESSION['paypwd'] = adminPayPasswordSessionValue($conf['admin_paypwd']);
		exit('{"code":0,"msg":"ok"}');
	}else{
...
case 'transfer':
	...
	if(!isset($_SESSION['paypwd']) || $_SESSION['paypwd']!==adminPayPasswordSessionValue($conf['admin_paypwd']))exit('{"code":-1,"msg":"支付密码错误，请返回重新进入该页面"}');
